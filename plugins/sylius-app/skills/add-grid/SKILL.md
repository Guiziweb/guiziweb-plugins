...
- Whether to include a `show` action (requires a show route — run `/sylius-app:add-routes` with a `show` operation if missing)

## 3. Grid skeleton

Grid name convention: `app_admin_{model_snake}`.
...
- `label: app.grid.{model_snake}.{field}` — translation key
...
- [ ] `bin/console sylius:debug:grid app_admin_{model_snake}` prints the grid definition (fields, filters, actions as declared)
- [ ] `bin/console debug:translation {locale} --domain=messages 2>&1 | grep 'app.grid.{model_snake}'` lists every label you added.

## Next steps

- Add admin routes referencing this grid → `/sylius-app:add-routes`
